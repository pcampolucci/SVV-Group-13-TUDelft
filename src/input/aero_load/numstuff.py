...
z = np.linspace(0, -0.547, 1000)  #np.linspace(0, -ca, -0.01)
loadiny =0
loadsety = []
j = 0
...
for i in z:
    ...
    if i <= zstuff[j] and i > zstuff[j+1]:
            #print(i)
            loadiny = zstuff[j] + coeffs[0][j]*(i-zstuff[j])+coeffs[1][j]*(i-zstuff[j])**2+coeffs[2][j]*(i-zstuff[j])**3
