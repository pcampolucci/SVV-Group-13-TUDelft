...
Sz= 91444 #[-142927.31474695,   50482.49366224 ,  47337.02570082   ,28483.05709263,
   #81931.43545731  ,-47482.16612374,  -40350.67118898,  -50588.59448186,
  #-52326.54567551 , 109083.27249464]
Sy= 73532 #[-40781.69979753, -24640.65650464, -23940.74388519, -27257.66312859,
  #21286.86468467  ,14219.17231002 , 13295.95216353,   8606.45625117,
 #  9175.06044528 , 90178.57730738]
...
T_lst = [ 660.44862356,  359.64170576,  278.01919764, -252.7818348 , 3979.4813368,
 -186.49317495 , 100.51646629 ,-276.95420611 ,-314.78817203, -839.02635342]   #change to actual torque distribution along the span for a number of points based on Willems code   
...
My_lst = [ 1279.48835396, 6219.7912426 , 17825.58106292 ,29427.9314608,
 48893.12680388 ,54825.33865843, 39017.58694589 ,24526.26153543,
 10348.0636794 ,  1101.02127179]          #to be changed and computed by a function in Willems code, just a single value for a spefic x location
Mz_lst = [   390.08069176 , -3988.5854398,  -11680.01329549 ,-18662.70792584,
 -21746.73532002, -13060.85756981, -10813.19731313 , -7099.87885604,
  -2946.29712559 ,   743.22999279]          #to be changed computed by a function in Willems code, just a single value for a spefic x location
...
#        sigma_xx_1 = [My[j]*(zco1[i] - zc)/Iyy + Mz[j]*yco1[i] for i in range(len(zco1))]            #zc is zlocation of centroid, compute y- and z-location of the middle of each section where the shear flow is computed
#        print(sigma_xx_1)
#        sigma_xx_2 = [My[j]*(zco2[i] - zc)/Iyy + Mz[j]*yco2[i] for i in range(len(zco2))]
#        sigma_xx_3 = [My[j]*(zco3[i] - zc)/Iyy + Mz[j]*yco3[i] for i in range(len(zco3))]
#        sigma_xx_4 = [My[j]*(zco4[i]- zc)/Iyy + Mz[j]*yco4[i] for i in range(len(zco4))]
#        sigma_xx_5 = [My[j]*(zco5[i] - zc)/Iyy + Mz[j]*yco5[i] for i in range(len(zco5))]
#        sigma_xx_6 = [My[j]*(zco6[i] - zc)/Iyy + Mz[j]*yco6[i] for i in range(len(zco6))]
...
#print(Von_Mises_stress_distribution_at_every_x_loc[1])
...
def plot_shear_stress(zco,yco,total_shear_stress_distribution_at_every_x_loc):
    n = 0          
    plt.scatter(zco1,yco1,c=total_shear_stress_distribution_at_every_x_loc[n],cmap='jet')
   
    plt.xlabel('z[m]', fontsize=16)
    plt.ylabel('y[m]', fontsize=16)
    #plt.legend()
    c=plt.colorbar()
    c.set_label('Shear Stress [N/m^2]')
    plt.title('Shear Stress Distribution')
    #plt.savefig("flow.png")
    plt.show()
#plot_shear_stress(zco1,yco1,total_shear_stress_distribution_at_every_x_loc)
  
def plot_Direct_stress_distribution(zco,yco,Direct_stress_distribution):
    n = 0          
    plt.scatter(zco1,yco1,c=direct_stress_distribution[n],cmap='jet')
   
    plt.xlabel('z[m]', fontsize=16)
    plt.ylabel('y[m]', fontsize=16)
    #plt.legend()
    c=plt.colorbar()
    c.set_label('Direct Stress [N/m^2]')
    plt.title('Direct Stress Distribution')
    #plt.savefig("flow.png")
    plt.show()
plot_Direct_stress_distribution(zco1,yco1,Von_Mises_stress_distribution_at_every_x_loc)
#def plot_Von_Mises_stress_distribution(zco,yco,Von_Mises_stress_distribution_at_every_x_loc):
#    n = 2          
...
#    #plt.savefig("flow.png")
#    plt.show()
#plot_Von_Mises_stress_distribution(zco1,yco1,Von_Mises_stress_distribution_at_every_x_loc)
#
#
#
#print(len(Von_Mises_stress_distribution_at_every_x_loc[2]))



x_axis = np.arange(0,l_a,l_a/len(T_lst))
plt.plot(x_axis, twist_lst)
plt.show()
