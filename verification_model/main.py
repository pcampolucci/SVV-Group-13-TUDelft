...
defl_1, defl_2, defl_3 = aileron.eval(x)       # Compute the three deflections
slope_1, slope_2, slope_3 = aileron.fdeval(x)     # Compute their their first order derivative
...
# Compute the loading
shear_y = aileron.Sy(x)               # Compute the shear force in y
shear_z = aileron.Sz(x)               # Compute the shear force in z
moment_y = aileron.My(x)               # Compute the moment around the y-axis
moment_z = aileron.Mz(x)               # Compute the moment around the z-axis
torque = aileron.T(x)                # Compute the torque
distr_torque = aileron.tau(x)              # Compute the distributed torque
...
str_list = ["defl_1", "defl_2", "defl_3", "slope_1", "slope_2",
            "slope_3", "shear_y", "shear_z", "moment_y", "moment_z", "torque", "distr_torque", "von_mises"]

von_mis_lst = [c1, c2, c3, c4, c5, c6]

val_list = [defl_1, defl_2, defl_3, slope_1, slope_2,
            slope_3, shear_y, shear_z, moment_y, moment_z, torque, distr_torque]

for out in range(len(str_list)):
    f = open(str_list[out] + ".txt", "w")

    if str_list[out] == "von_mises":
        for item in von_mis_lst:
            f.write('%s,' % max(item))

    else:
        for item in val_list[out]:
            f.write('%s,' % item)
